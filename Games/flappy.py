...
# loading Games/Images/Flappy
background = pygame.image.load('Games/Images/Flappy/background2.jpg')
base = pygame.image.load('Games/Images/Flappy/base3.jpg')
button = pygame.image.load('Games/Images/Flappy/restart.png')
...
class FlappyBird(pygame.sprite.Sprite):
    ...
    def __init__(self, x_coordinate, y_coordinate):
        ...
        for i in range(1, 4):
            # loading the sprite bird Games/Images/Flappy from the directory
            # using the load() function of the pygame.image module
            image = pygame.image.load(f'Games/Images/Flappy/bird_{i}.png')
    ...
    def update(self, birdFlying, gameOver):
        ...
        if birdFlying == True:
            ...
            if self.velocity > 8.5:
                self.velocity = 8.5
                # if the rectangle's bottom is less than SCREEN_HEIGHT - 180
            # then increment its y-axis value by velocity's integer value
            if self.rect.bottom < SCREEN_HEIGHT - 180:
...
class Pipe(pygame.sprite.Sprite):
    # defining an initializing function
    def __init__(self, x_coordinate, y_coordinate, position, pipeGap):
        pygame.sprite.Sprite.__init__(self)
        # loading the sprite pipe image from the directory
        # using the load() function of the pygame.image module
        self.image = pygame.image.load('Games/Images/Flappy/pipe.png')
...
def play_flappy(start_menu, p, t):
    ...
    restartButton = Button(SCREEN_WIDTH / 2 - 302 / 2, SCREEN_HEIGHT / 2 - 302 / 2, button)
    ...
    while game_run:
        ...
        # drawing the base
        display_screen.blit(base, (baseScroll, SCREEN_HEIGHT - 180))
        ...
        if bird.rect.bottom >= SCREEN_HEIGHT - 180:
            gameOver = True
            birdFlying = False
        ...
        if gameOver == False and birdFlying == True:
            ...
            if timeNow - lastPipe > pipeFrequency:
                ...
                bottomPipe = Pipe(SCREEN_WIDTH, int(SCREEN_HEIGHT / 2) + pipeHeight, -1, pipeGap)
                topPipe = Pipe(SCREEN_WIDTH, int(SCREEN_HEIGHT / 2) + pipeHeight, 1, pipeGap)
        ...
        if gameOver == True:
            if restartButton.draw() == True:
                gameOver = False
                playerScore = resetGame(pipeGroup, bird)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and birdFlying == False and gameOver == False:
                    birdFlying = True
